...
python setup.py
重要：請使用手機掃描產生的 2fa_setup_qr.png，完成後請務必刪除該圖片。
...
注意：首次進入系統後，請務必修改預設密碼。

資料庫欄位
項目：目標網站或系統名。

使用者帳號：Login Username。
...
⚠️ 安全警告
手動解鎖機制：本系統設計為「每次服務重啟後，必須手動登入一次」以解鎖資料庫。這是為了避免金鑰以明文存在伺服器上。

備份安全：backups/ 內的檔案雖已加密，但仍建議定期搬移至離線裝置儲存。

2FA 密鑰：請妥善保存 config.yaml 中的 otp_secret，萬一手機遺失，需依此密鑰恢復。

Developed for secure management at https://secure.rogerlin.xyz:8888
